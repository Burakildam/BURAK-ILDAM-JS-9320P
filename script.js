...
const filteredCharacters = document.getElementById("filterCharacters");
...
function toggleRender() {
  ...
  if (charactersRender) {
    ...
    characters.forEach((karakter) => {
      charactersDiv.innerHTML += `<div class="col-lg-4 col-sm-6 mb-5"  data-id="${karakter.homeworld}">
        <div class="card text-white border-light" style="width: 18rem">
    ...
    toggleButton.textContent = "Karakteri  Gizle";
  }
}
...
radioFilterHtml.addEventListener("change", (radio) => {
  if (radio.target.name == "exampleRadioInput") {
    const selectedRadioId = radio.target.value;
    filterCharacters(selectedRadioId);
  }
});
